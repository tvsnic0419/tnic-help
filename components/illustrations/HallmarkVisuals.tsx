 ...
 * - Tries to load premium Canva PNG first (when illustrationV2 is provided)
 * - On error / missing file: falls back gracefully
 * - Designed so final premium images can replace placeholders with zero code changes
 */
export const HallmarkIllustration: React.FC<HallmarkIllustrationProps> = ({
  ...
  // Try high-quality v2 image first
  if (illustrationV2 && !imgError) {
    return (
      <div className={`relative w-full overflow-hidden rounded-2xl border border-white/10 bg-[#0a0f1a] ${className}`}>
        <img
        ...
        <div className="absolute inset-0 bg-gradient-to-b from-black/10 via-transparent to-black/25 pointer-events-none rounded-2xl" />
      </div>
    );
  }

  // Fallback to provided visual or legacy SVG
  if (fallbackVisual) {
    return <>{fallbackVisual}</>;
  }

  // Smart fallback based on slug (for missing premium images)
  return <HallmarkIllustrationFallback slug={slug} className={className} />;
};

// High-quality placeholder SVGs for missing premium illustrations
function HallmarkIllustrationFallback({ slug, className = '' }: { slug: string; className?: string }) {
  const baseClasses = `w-full rounded-2xl border border-white/10 bg-[#0a0f1a] p-6 flex items-center justify-center ${className}`;

  switch (slug) {
    case 'disabled-autophagy':
      return (
        <div className={baseClasses}>
          <div className="text-center">
            <div className="mx-auto mb-3 h-px w-12 bg-white/30" />
            <p className="font-mono text-[10px] tracking-[3px] text-accent-emerald mb-1">HALLMARK 5</p>
            <p className="text-lg font-semibold text-white/90 mb-1">Disabled Autophagy</p>
            <p className="text-xs text-white/60 max-w-[220px]">Cellular cleanup slows. Damaged proteins and mitochondria accumulate.</p>
            <div className="mt-4 text-[10px] text-accent-emerald/70">Premium illustration coming soon</div>
          </div>
        </div>
      );
    ...
    case 'dysbiosis':
      return (
        <div className={baseClasses}>
          <div className="text-center">
            <div className="mx-auto mb-3 h-px w-12 bg-white/30" />
            <p className="font-mono text-[10px] tracking-[3px] text-accent-amber mb-1">HALLMARK 11</p>
            <p className="text-lg font-semibold text-white/90 mb-1">Dysbiosis</p>
            <p className="text-xs text-white/60 max-w-[220px]">Gut microbiome imbalance drives systemic inflammation and metabolic dysfunction.</p>
            <div className="mt-4 text-[10px] text-accent-amber/70">Premium illustration coming soon</div>
          </div>
        </div>
      );
    ...
    default:
      return (
        <div className={`flex items-center justify-center rounded-2xl border border-white/10 bg-[#111827] text-[#9ca3af] text-sm p-8 ${className}`}>
...
// Keep existing SVG components for backward compatibility
export const GenomicInstabilityVisual: React.FC<HallmarkVisualProps> = ({
  ...
  return (
    ...
    >
      ...
      <path d="M180 40 Q200 80 180 120 Q200 160 180 200" stroke={accent} strokeWidth="8" strokeLinecap="round" fill="none" />
      <path d="M210 40 Q190 80 210 120 Q190 160 210 200" stroke={accent} strokeWidth="8" strokeLinecap="round" fill="none" />
